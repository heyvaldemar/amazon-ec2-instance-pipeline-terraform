# An EC2 instance, done properly: Terraform

[![Terraform Verification](https://github.com/heyvaldemar/amazon-ec2-instance-pipeline-terraform/actions/workflows/terraform-verification.yml/badge.svg?branch=main)](https://github.com/heyvaldemar/amazon-ec2-instance-pipeline-terraform/actions/workflows/terraform-verification.yml)
[![OpenSSF Best Practices](https://www.bestpractices.dev/projects/14917/badge)](https://www.bestpractices.dev/projects/14917)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

This repository deploys a single EC2 instance with its own VPC, security group, generated SSH key pair and Elastic IP, plus a self-provisioned Terraform state backend (S3 + DynamoDB + KMS): the smallest honest starting point for an AWS estate. Flat, numbered `.tf` files, no modules to chase, every provider locked to an exact build.

## What it creates

| Area | Resources |
|---|---|
| Networking | `aws_vpc`, `aws_subnet`, `aws_internet_gateway`, `aws_eip`, `aws_route_table`, `aws_route_table_association`, `aws_flow_log`, `aws_security_group` |
| Compute | `aws_instance`, `aws_key_pair`, `tls_private_key` |
| State backend and encryption | `aws_s3_bucket` ×3, `aws_s3_bucket_versioning` ×3, `aws_s3_bucket_server_side_encryption_configuration` ×3, `aws_s3_bucket_public_access_block` ×3, `aws_s3_bucket_policy` ×2, `aws_s3_bucket_logging`, `aws_dynamodb_table`, `aws_kms_key` ×2, `aws_kms_alias` ×2, `random_id` |

18 variables, 5 outputs. Every variable has a description and a default in `00-variables.tf`.
...
```bash
# 1. Clone
git clone https://github.com/heyvaldemar/amazon-ec2-instance-pipeline-terraform
cd amazon-ec2-instance-pipeline-terraform
...
Every release from v1.1.5 on carries three files made on GitHub's runner with a short-lived identity and no stored key: `amazon-ec2-instance-pipeline-terraform-<tag>.tar.gz`, a `git archive` of exactly the tree the tag points at; `amazon-ec2-instance-pipeline-terraform-<tag>.tar.gz.sigstore.json`, a keyless [Sigstore](https://www.sigstore.dev/) signature over it; and `amazon-ec2-instance-pipeline-terraform-<tag>.intoto.jsonl`, [SLSA](https://slsa.dev/) build provenance from the SLSA generator. To check them with nothing from this repository trusted:

```bash
cosign verify-blob amazon-ec2-instance-pipeline-terraform-<tag>.tar.gz \
  --bundle amazon-ec2-instance-pipeline-terraform-<tag>.tar.gz.sigstore.json \
  --certificate-identity-regexp '^https://github.com/heyvaldemar/amazon-ec2-instance-pipeline-terraform/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

slsa-verifier verify-artifact amazon-ec2-instance-pipeline-terraform-<tag>.tar.gz \
  --provenance-path amazon-ec2-instance-pipeline-terraform-<tag>.intoto.jsonl \
  --source-uri github.com/heyvaldemar/amazon-ec2-instance-pipeline-terraform
```
...
The [Terraform Verification](https://github.com/heyvaldemar/amazon-ec2-instance-pipeline-terraform/actions/workflows/terraform-verification.yml?query=branch%3Amain) workflow runs on every push, pull request, and weekly: `terraform fmt -check`, `terraform init -lockfile=readonly`, `terraform validate`, `tflint`, and actionlint on the workflow itself.

**What the configuration promises is tested, not just parsed.** [`tests/posture.tftest.hcl`](tests/posture.tftest.hcl) plans the configuration with its default variables against mocked providers, with no AWS account and no credentials involved, and makes 16 assertions about what that plan would build: KMS keys rotate, no bucket can be made public, buckets are encrypted with KMS and versioned, the state lock table is encrypted and recoverable to a point in time, and each resource type below it keeps its own promise. [`tests/plant_violations.py`](tests/plant_violations.py) then breaks those promises one at a time on a copy of the configuration, 13 ways listed in [`tests/plants.tsv`](tests/plants.tsv), and fails the run if the test stays green through any of them. Both run in CI on every push.
